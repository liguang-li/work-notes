...
struct kmem_cache {
        ...
        unsigned long min_partial; //限制struct kmem_cache_node中的partial链表slab的数量,大于
    								这个mini_partial的值，那么多余的slab就会被释放
        int size;               /* The size of an object including meta data */
        int object_size;        /* The size of an object without meta data */
        int offset;             /* Free pointer offset. */ //每个object在没有分配之前,完全可以
    							   在每个object中存储下一个object内存首地址，就形成了一个单链表,
    							   offset就是存储下个object地址数据相对于这个object首地址的偏移
        int cpu_partial;        /* Number of per cpu partial objects to keep around */ 									//per_cpu partial中所有slab的free object的数量的最大值，超过这
    							  个值就会将所有的slab转移到kmem_cache_node的partial链表
        ...
        struct kmem_cache_order_objects min; //内存不足时,采用min分配,参考oo
...
![image-20191209142123039](slub.assets/image-20191209142123039.png)
...
struct page {
	...
	defined(CONFIG_HAVE_ALIGNED_STRUCT_PAGE)
			...
			struct {
				union {
					struct { /* SLUB */
						unsigned inuse:16;	//used obj nums
						unsigned objects:15;	//total obj nums
						unsigned frozen:1;	//被某CPU独占
					};
				};
...
![image-20191209173834980](slub.assets/image-20191209173834980.png)
...
* slab缓冲池包含的页数是由oo决定的.oo拆分为两部分,低16位代表一个slab缓冲池中obj的数量,高16位代表slab包含的页数.即从伙伴系统得到2^(oo>>16)pages的内存.
* per_cpu->freelist: 本地缓冲池,freelist成员指向第一个可用内存obj首地址.
* per_cpu->partial:
* node->partial: node中的slab是共享的,per_cpu->partial是独占的.

## kmem_cache_alloc
![img](http://www-x-wowotech-x-net.img.abc188.com/content/uploadfile/201802/10fb1519305301.png)

![image-20191216143651509](slub.assets/image-20191216143651509.png)

![image-20191216143711436](slub.assets/image-20191216143711436.png)

![image-20191216143748178](slub.assets/image-20191216143748178.png)

![image-20191216143526719](slub.assets/image-20191216143526719.png)

## kmem_cache_free

![image-20191216143858530](slub.assets/image-20191216143858530.png)
